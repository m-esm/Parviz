...
a properly meshing pair:
  ...
  stl/neck/tilt_worm_real.stl   single-start trapezoidal-thread worm, axis Y, centered
...
Design derivation (all mm / degrees; see docs/WORM.md for the full story):
  - module 1.25 and z=12 fix the wheel pitch radius at 7.5 and the worm axial
    pitch (= lead, single start) at pi*m = 3.92699.
  ...
  - Lead angle = atan(lead / (2*pi*4.4)) = 8.085 deg. Self-locks for printed
    plastic (needs mu > tan(lambda)*cos(alpha) = 0.129; PLA-PLA dry ~0.3-0.4).
...
def build_wheel():
    ...
    twist = np.degrees(FACE_W * np.tan(LEAD_ANGLE) / R_P)     # 7.60 deg
    man = m3.Manifold.extrude(cs, FACE_W, n_divisions=24, twist_degrees=HAND * twist)
    mm = man.to_mesh()
...
def build_worm():
    ...
    # sweep the rib along the helix, overshooting the trim slab by ~a pitch each end
    ...
    for t in ts:
        ...
        ring = [(r * np.sin(t), yc + dy, r * np.cos(t)) for dy, r in prof]
        rings.append(ring)
    ...
    thread.fix_normals()
    assert thread.is_volume, "thread sweep not a volume"
    ...
    worm = trimesh.boolean.intersection(
        [trimesh.boolean.union([core, thread], engine="manifold"), slab],
        engine="manifold")
    return worm
...
def main():
    ...
    print(f"wrote {OUT_WHEEL} and {OUT_WORM}")
