...
- Print supports: the neck's tilt-stop posts no longer start mid-air. Each
  outboard half (|x| 25.5..32, behind the post rear face) has a printed bridge
  that fuses into the leg, so the on-its-back orientation (export R(X,90))
  lands that half on material instead of a slicer support scar on the homing
  face. Still paint a support enforcer over the head_back stop fins (55°
  overhangs feeding a ~0.9 mm homing margin; treat the first stall-home angle
  as calibration). Print the track keepers lying on their 13.3×1.9 bar face
  so the slot-critical 1.9 width is XY-accurate.
