...
What gets a stand-in (all threadless -- push/press fits, see per-part notes):
  ...
  hw_m8_nut    x4   AF13 hex x5 (the tower nut cages grip AF13+0.4), bore Ø7.8 =
                    0.2 press on the shank. Front tensioners: push to tension, add a
                    drop of CA if it creeps -- a push nut can't hold real preload.
  hw_m8_washer x4   Ø14.4/Ø8.4 x 1.5.
  hw_f688_bushing x8 flanged BUSHING replacing the F688ZZ bearing: OD 15.85 (easy fit
                    in the Ø15.95 press seat -- steel gets the press, plastic slides),
                    bore Ø8.3 running on the Ø8.0 shank, Ø18.2 flange. Idlers become
                    plain bearings: fine at desk speeds, grease helps.
  ...
  hw_seam_dowel x4  Ø4x12 seam dowels (chassis lower seams, bezel halves).
  hw_foot_pin   x2   Ø3x6 trim_neckfoot locator pins.
...
stance); nuts/washers/bushings lie flat; everything is NOSUP. Bolt shanks printed
vertically are weak in bending at layer lines -- at this robot's ~1 N/wheel loads
that's fine, but don't drive off a desk on plastic axles.
"""
import math
...
STANDINS = {
    ...
    "hw_seam_dowel": (lambda: _zmin0(cyl(2.0, 12.0)), 4),
